...
1. Search for `'ripple'` (inside single quotations) to capture the text domain.
2. Search for `ripple_` to capture all the function names.
3. Search for <code>&nbsp;_s</code> (with a space before it) to capture DocBlocks.
<<<<<<< HEAD
4. Search for `ripple-` to capture prefixed handles.
=======
4. Search for `_s-` to capture prefixed handles.
5. Search for `Text Domain: _s` in style.css.
>>>>>>> 5199a56b05109afdf9427deb6cce70d7936c4698

OR

* Search for: `'ripple'` and replace with: `'megatherium'`
* Search for: `ripple_` and replace with: `megatherium_`
