<?php
/**
 * Dev Icon Sidebar.
 *
 * @package Ripple
 */
?>
	<div id="secondary" class="tab-pane widget-area text-center shaded" role="complementary">
  		<section>
	  		<div class="wrapper">
	    		<h3>Check out some of the great features Ripple has for developers.</h3>
				<aside class="col-lg-3">
					<a href="<?php echo esc_url( home_url( '/' ) ); ?>protocol/#powerful-apis">
						<div class="icons open">	
						</div>
						<h4 class="icons-title">Open Source</h4>
				    	<p>Code open to download, modify, and run.</p>
					 </a>
				</aside>

				<aside class="col-lg-3">
					<a href="<?php echo esc_url( home_url( '/' ) ); ?>payment-network/#peer-to-peer">
					<div class="icons p2p">
					</div>
				    	<h4 class="icons-title">Peer-to-Peer</h4>
				    	<p>Transactions are direct and P2P.</p>
					 </a>
				</aside>

				<aside class="col-lg-3">
					<a href="<?php echo esc_url( home_url( '/' ) ); ?>payment-network/#micropayments">
						<div class="icons micro">	
						</div>
					    <h4 class="icons-title">Micropayments</h4>
					    <p>Payments of less than $0.01 possible.</p>
					</a>
				</aside>

				<aside class="col-lg-3">
					<a href="<?php echo esc_url( home_url( '/' ) ); ?>protocol/#open-source">
						<div class="icons powerful">	
						</div>
				    	<h4 class="icons-title">Powerful and open APIs</h4>
				    	<p>No fees. No sign-up or approval. Build apps that interact directly with a real-money network.</p>
					 </a>
				</aside>
			</div><!-- .wrapper -->	
		</section>
	</div><!-- #secondary -->


