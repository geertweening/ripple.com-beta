...
FAQ ITEM
	...
	[faq title="title"]content[/faq]

TARGET SECTION

	[target_section target="one"]content[/target_section]
...
function target_section($atts, $content=null){
	extract(shortcode_atts( array( 
							'target' => '' 
							), $atts ));
	
	return '<article id="'.$target.'-target">'.do_shortcode($content).'</article>';
							
							
}
add_shortcode('target_section', 'target_section');
...
function video( $atts, $content = null ) {
	...
	$out .='<iframe  src="'.$videoURL.'" width="'.$w.'" height="'.$h.'" ></iframe>';
	$out .= "</div></div>";
...
function gmap( $atts, $content = null ) {
	...
	extract(shortcode_atts(array('lat' => '-12.043333',
								 'lng' => '-77.028333',
								 'latm'  => '-12.043333',
								 'lngm' => '-77.028333' ),$atts));
	...
	$out = '<div id="map"></div>';
	$out .= '<script type="text/javascript" src="http://maps.google.com/maps/api/js?sensor=true"></script>';
	$out .= '<script type="text/javascript" src="http://maps.google.com/maps/api/js?sensor=true"></script>';

	$out .= '<script src="'. get_template_directory_uri() .'/js/gmaps.js"></script>';
