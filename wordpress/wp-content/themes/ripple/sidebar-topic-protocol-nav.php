<?php
/**
 * The Sidebar topic-protocol-nav.
 *
 * @packageRipple
 */
?>
		<nav id="scrollspy-nav" class="guide-nav pull-left" role="navigation">
			<h5 class="nav-heading">Topic: Protocol</h5>
       		<ul class="nav nav-list">
       			<li><a href="#math-based">Math-Based</a></li>
       			<li><a href="#distributed">Distributed</a></li>
				<li><a href="#the-ledger">The Ledger</a></li>
				<li><a href="#secure">Secure</a></li>
				<li><a href="#advanced-security-measures">Advanced Security Measures</a></li>
				<li><a href="#powerful-apis">Powerful APIs</a></li>
				<li><a href="#open-apis">Open APIs</a></li>
				<li><a href="#easy-integration">Easy Intergration</a></li>
				<li><a href="#open-source-coming-soon">Open Source (coming soon)</a></li>
				<li><a href="#open-client">Open Client</a></li>
				<li><a href="#smart-contracts-coming-soon">Smart Contracts (coming soon)</a></li>
				<li><a href="#federation-api">Federation API</a></li>
				<li><a href="#bridge-protocol">Bridge Protocol</a></li>
				<li><a href="#dynamic-scalability">Dynamic Scalability</a></li>
				<li><a href="#how-to-guides">HOW TO GUIDES</a></li>
       		</ul>
		</nav>
